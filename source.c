...
void go_go(char *root_path, char *path, int fd)
{
  ...
  while (curr = readdir(dir)) {
    if (curr->d_type != 4) {
      //printf("\n%s\n", curr->d_name);
      char *name = calloc(strlen(curr->d_name) + 1, 1);

      strcpy(name, curr->d_name);
      char *path_new = cat(path, name);

      free(name);
      int len = strlen(path_new) - strlen(root_path);

      assert(write(fd, &len, sizeof(int)) != -1);
      assert(write(fd, path_new + strlen(root_path), len) != -1);
